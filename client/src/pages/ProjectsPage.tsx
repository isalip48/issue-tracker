...
export const ProjectsPage = () => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to delete this project? This will fail if there are issues linked to it.")) {
    ...
    <div className="flex-1 flex flex-col w-full animate-fade-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {isLoading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-48 rounded-3xl bg-secondary/20 animate-pulse border border-border/50" />
          ))
        ) : projects?.length === 0 ? (
          <div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground max-w-xs">Create your first project to start organizing your issues effectively.</p>
            </div>
            <button
              onClick={() => handleOpenDialog()}
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="mt-6 pt-5 border-t border-border/40 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground font-semibold">{project.createdBy?.name || "Unknown"}</span>
                  </div>
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={handleCloseDialog} />
          ...
          <div className="relative bg-card w-full max-w-md border border-border/80 rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-muted-foreground font-medium mb-8">
              {editingProject ? "Update existing project details" : "Create a new container for your issues"}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  {createMutation.isPending || updateMutation.isPending ? "Processing..." : editingProject ? "Save Changes" : "Create Project"}
                </button>
              </div>
            </form>
          </div>
